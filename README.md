...
```
### Here is an example of a new test case in the BoaTest class that tests the lengthInInches() method:
```
...
```
This new test case checks that lengthInInches() method returns expected value when called on each of the Boa objects created in the setUp() method. It uses assertEquals() method to compare the expected value to the actual value returned by the lengthInInches() method. The @Test annotation indicates that this is a test method that should be run by JUnit.
</br>
